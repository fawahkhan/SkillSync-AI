        ...
        {typeof data.image === 'object' && (
            <div className='flex flex-col gap-1 pl-4 text-sm'>
                ...
                <label className='relative inline-flex items-center cursor-pointer text-slate-900 gap-3 '>
                    ...
                    <div className="w-9 h-5 bg-slate-300 rounded-full peer peer-checked:bg-teal-600 transition-colors duration-200">
                    </div>
        ...
        {fields.map((field) => {
            ...
            return(
                <div key={field.key} className='space-y-1 mt-5'>
                    ...
                    <input type= {field.type} value={data[field.key] || ""} onChange={(e)=>{handleChange(field.key , e.target.value)}} className='mt-1 w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors text-sm' placeholder ={`Enter your ${field.label.toLowerCase()}`} required={field.required}/>
                </div>
            )
        })}
    </div>
