        ...
        {showCreateResumes && (
          <form onSubmit={createResume} onClick={()=>{setShowCreateResumes(false)}} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
            <div onClick={e=>{e.stopPropagation()}} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6 '> 
              ...
              <input onChange={(e)=>{setTitle(e.target.value)} } value = {title} type="text" placeholder='Enter resume title' className='w-full px-4 py-2 mb-4 focus:border-teal-600 ring-teal-600' required />
              <button className='w-full py-2 bg-teal-600 text-white rounded hover:bg-teal-700 transition-colors'>Create Resume </button>
              {/* icon to close popup */}
        ...
        {showUploadResumes && (
          <form onSubmit={uploadResume} onClick={()=>{setShowUploadResumes(false)}} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
            <div onClick={e=>{e.stopPropagation()}} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6 '> 
              ...
              <input onChange={(e)=>{setTitle(e.target.value)} } placeholder='Enter resume title' value = {title} type="text" accept='.pdf,.doc,.docx' className='w-full px-4 py-2 mb-4 focus:border-teal-600 ring-teal-600' required />
              <div>
                <label htmlFor="resume-input" className='block text-sm text-slate-700'>
                  Select resume file
                  <div className='flex flex-col items-center justify-center gap-2 border group text-slate-400 border-dashed rounded-md p-4 py-10 my-4 hover:border-teal-500 hover:text-teal-700 cursor-pointer transition-colors'>
              ...
              <button disabled={isLoading} className='w-full py-2 bg-teal-600 text-white rounded hover:bg-teal-700 transition-colors flex items-center justify-center gap-2'>
        ...
        {editresumeId && (
          <form onSubmit={editTitle} onClick={()=>{seteditResumeId('')}} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
            <div onClick={e=>{e.stopPropagation()}} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6 '> 
              ...
              <input onChange={(e)=>{setTitle(e.target.value)} } value = {title} type="text" placeholder='Enter resume title' className='w-full px-4 py-2 mb-4 focus:border-teal-600 ring-teal-600' required />
              <button className='w-full py-2 bg-teal-600 text-white rounded hover:bg-teal-700 transition-colors'>Update </button>
              {/* icon to close popup */}
